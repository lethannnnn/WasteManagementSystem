...
function App() {
  ...
  const [collectorOverview, setCollectorOverview] = useState<CollectorOverview>({
    activeCollectors: 0,
    avgPerformance: 0,
    routesCompleted: 0,
    avgRating: 0
  ...
  const [realTimeTracking, setRealTimeTracking] = useState<RealTimeTracking>({
    collectors: []
  });
  ...
  const fetchUsers = async () => {
    try {
      ...
      const formattedUsers = usersData.map(user => ({
        ...
        name: user.full_name,
        email: user.email,
        type: (user.donors ? 'Donor' : user.collectors ? 'Collector' : user.sponsors ? 'Sponsor' : 'Admin') as 'Donor' | 'Collector' | 'Sponsor' | 'Admin',
        status: user.collectors ? user.collectors[0]?.status : 'Active',
        points: user.donors ? user.donors[0]?.points : 0,
  ...
  const fetchCollectorPerformance = async () => {
    try {
      ...
      const collectorStats: { [key: string]: { name: string; completedPickups: number; totalTime: number } } = {};
      if (completedPickups) {
        completedPickups.forEach(pickup => {
          const collectorId = pickup.collector_id;
          if (!collectorStats[collectorId]) {
            collectorStats[collectorId] = {
              name: `${pickup.collectors[0]?.users[0]?.first_name} ${pickup.collectors[0]?.users[0]?.last_name}`,
              completedPickups: 0,
              totalTime: 0
            };
          }
      ...
      const topPerformers = Object.entries(collectorStats)
        .map(([collectorId, stats]) => ({
          ...
          routeInfo: `${stats.completedPickups} pickups completed`,
    ...
    try {
      ...
      const routesList = routes?.slice(0, 10).map(route => {
        ...
        const collectorName = assignment ? 
          `${assignment.collectors[0]?.users[0]?.first_name} ${assignment.collectors[0]?.users[0]?.last_name}` : 
          'Unassigned';
        
        // Count pickups for this route's collector
        const routePickups = pickups?.filter(pickup => 
          pickup.collector_id === assignment?.collectors[0]?.collector_id
        ).length || 0;
        ...
        return {
          id: route.route_id,
          name: collectorName,
          status: route.status as 'active' | 'completed' | 'pending',
  ...
  const fetchRealTimeTracking = async () => {
    try {
      ...
      const trackingData = collectorsData?.map(collector => {
        ...
        if (assignment && route) {
          status = route[0]?.status === 'Active' ? 'active' : 'idle';
          routeInfo = `Route ${route[0]?.route_id}`;
        }
    ...
    try {
      ...
      pickupsData.forEach(pickup => {
        pickup.pickup_items?.forEach(item => {
          const category = item.item_categories?.[0]?.category_name || 'Other';
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        const monthlyMaterialBreakdown: { [key: string]: number } = {};
        ...
        Object.keys(monthlyMaterialBreakdown).forEach(material => {
          const weight = (monthlyMaterialBreakdown as { [key: string]: number })[material];
          materials[material] = {
            weight,
        ...
        monthlyData.push({
          ...
          uniqueDonors: new Set(monthPickups.map(p => p.pickup_id)).size
        });
      }
      
      // Calculate environmental impact metrics
      const environmentalImpact = {
        co2Saved: parseFloat((totalWeight * 0.75).toFixed(1)), // 0.75 kg CO2 saved per kg recycled
        ...
        landfillDiverted: parseFloat((totalWeight / 1000).toFixed(1)) // tons diverted from landfill
      };
      
      // Calculate regional breakdown
      const regionalData: { [key: string]: { users: number; volume: number; pickups: number; growth: number } } = {};
      ...
      pickupsData.forEach(pickup => {
        const state = pickup.users?.[0]?.state || 'Unknown';
        if (regionalData[state]) {
      ...
      const regionalBreakdown = Object.entries(regionalData)
        .map(([state, data]: [string, { users: number; volume: number; pickups: number; growth: number }]) => ({
          state,
          ...data,
          volume: Math.round(data.volume),
          collections: data.pickups, // Use pickups as collections
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (profile?.user_type === 'admin') {
          // User session handled by Supabase
          setIsLoggedIn(true);
              ...
              <div className="metrics-grid">
                ...
                <div className="metric-card success">
                  ...
                  <div className="metric-info">
                    ...
                    <p className="metric-number">{dataLoading ? 'Loading...' : analytics ? `${((analytics.totalWeight || 0) / 1000).toFixed(1)} tons` : '0 tons'}</p>
                ...
                <div className="metric-card info">
                  ...
                  <div className="metric-info">
                    <h3>Total Pickups</h3>
                    <p className="metric-number">{dataLoading ? 'Loading...' : analytics ? (analytics.totalPickups || 0).toLocaleString() : '0'}</p>
            ...
            <div className="analytics-grid">
              ...
              <div className="chart-card large">
                <div className="chart-header">
                  ...
                  <div className="chart-stats">
                    <span className="stat-item">
                      <strong>{dataLoading ? 'Loading...' : analytics ? `${((analytics.totalWeight || 0) / 1000).toFixed(1)} tons` : '0 tons'}</strong> total recycled
                    </span>
                    <span className="stat-item growth">
                      ♻️ All time
                    </span>
                  ...
                  <div className="material-trends">
                    ...
                    <div className="trends-grid">
                      {Object.keys(analytics?.materialBreakdown || {}).filter(material => 
                        ...
                        const trend = (materialData > 0 ? 'stable' : 'stable') as 'increasing' | 'decreasing' | 'stable';
                        return (
                        ...
                        return (
                          <div key={index} className="collector-item">
                            ...
                            <div className="collector-info">
                              ...
                              <span className="stats">{performer.pickups || 0} pickups • {(performer.successRate || 0).toFixed(1)}% success</span>
                            </div>
                            <div className="collector-score">{(performer.efficiency || 0).toFixed(1)}</div>
                          </div>
                        );
                      })}
