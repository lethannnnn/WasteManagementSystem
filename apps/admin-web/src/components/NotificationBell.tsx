...
async function fetchNotifications() {
  const [pickupsRes, inquiriesRes] = await Promise.all([
    supabase
      .from('pickups')
  ...
  const pickups: PendingPickup[] = (pickupsRes.data ?? []).map(p => ({
    id:         p.pickup_id,
    donor:      (p.donors as any)?.users?.full_name ?? 'Unknown',
    created_at: p.created_at,
  }))

  const inquiries: PendingInquiry[] = (inquiriesRes.data ?? []).map(i => ({
    id:         i.id,
...
export default function NotificationBell() {
  ...
  const totalCount   = (data?.pickupCount ?? 0) + (data?.inquiryCount ?? 0)
